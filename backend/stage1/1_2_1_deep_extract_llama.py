#!/usr/bin/env python3
"""
Stage 1 - Step 2 Substep 1: Deep JSON Extraction with Llama 4 Maverick
Extract deep JSON from scanned PDFs for comparison with Claude Opus 4.5

Uses Llama 4 Maverick 17B Instruct for document analysis
Processes all scanned PDFs (excluding large files like tax_returns)
Saves individual JSON outputs per document in PAGE-WISE format

Naming: 1_2_1 = Stage 1, Step 2, Substep 1
"""
...
def process_loan_scanned_pdfs(loan_id: str, stage1_output_dir: Path, documents_dir: Path):
    """Process all scanned PDFs for a loan"""
    
    print("\n" + "="*80)
    print(f"🚀 STAGE 1 STEP 2.1: Deep JSON Extraction (Page-wise) - Scanned PDFs - {loan_id}")
    ...
    # Get scanned PDFs list
    scanned_files = categories['categories']['scanned_pdfs_ocr']['files']
    
    # Exclude large files
    EXCLUDED_FILES = ['tax_returns_65.pdf']
    files_to_process = [f for f in scanned_files if f not in EXCLUDED_FILES]
    
    print(f"\n📄 Found {len(scanned_files)} scanned PDFs")
    print(f"   Excluding: {EXCLUDED_FILES}")
    print(f"   Processing: {len(files_to_process)} PDFs\n")
    
    # Create output directory
    script_dir = Path(__file__).parent
    output_dir = script_dir / "output" / loan_id / "1_2_1_llama_extractions"
    ...
    for idx, filename in enumerate(files_to_process, 1):
        ...
        result = extract_document_pagewise(str(pdf_path), filename, client, pdf_type="scanned", dpi=150)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python 1_2_1_deep_extract_llama.py <loan_id>")
        print("\nExample:")
        print("  python 1_2_1_deep_extract_llama.py loan_1642451")
        print("\nPrerequisites:")
        ...
        print("\nOutput:")
        print("  Individual JSON files: backend/stage1/output/<loan_id>/1_2_1_llama_extractions/<filename>.json")
        print("  Summary file: backend/stage1/output/<loan_id>/1_2_1_llama_extractions/extraction_summary.json")
    ...
    # Run extraction
    summary = process_loan_scanned_pdfs(loan_id, stage1_output_dir, documents_dir)
