#!/usr/bin/env python3
"""
Stage 1 - Step 2 Substep 4: Deep JSON Extraction for Text PDFs Without Tables
Extract deep JSON from text-based PDFs without tables using Llama 4 Maverick

Uses Llama 4 Maverick 17B Instruct for document analysis
Processes all text PDFs without tables (simple text extraction candidates)
Saves individual JSON outputs per document in PAGE-WISE format

Naming: 1_2_4 = Stage 1, Step 2, Substep 4
"""
...
def process_loan_text_no_tables(loan_id: str, stage1_output_dir: Path, documents_dir: Path):
    """Process all text PDFs without tables for a loan"""
    
    print("\n" + "="*80)
    print(f"🚀 STAGE 1 STEP 2.4: Deep JSON Extraction (Page-wise) - Text PDFs No Tables - {loan_id}")
    ...
    # Get text PDFs without tables list
    text_no_tables_files = categories['categories']['text_pdfs_no_tables_text_extraction']['files']
    
    # No exclusions for these files (they're already simple)
    files_to_process = text_no_tables_files
    
    print(f"\n📄 Found {len(text_no_tables_files)} text PDFs without tables")
    print(f"   Processing: {len(files_to_process)} PDFs\n")
    
    # Create output directory
    script_dir = Path(__file__).parent
    output_dir = script_dir / "output" / loan_id / "1_2_4_llama_extractions"
    ...
    for idx, filename in enumerate(files_to_process, 1):
        ...
        result = extract_document_pagewise(str(pdf_path), filename, client, pdf_type="text_no_tables", dpi=150)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python 1_2_4_deep_extract_text_no_tables.py <loan_id>")
        print("\nExample:")
        print("  python 1_2_4_deep_extract_text_no_tables.py loan_1642451")
        print("\nPrerequisites:")
        ...
        print("\nOutput:")
        print("  Individual JSON files: backend/stage1/output/<loan_id>/1_2_4_llama_extractions/<filename>.json")
        print("  Summary file: backend/stage1/output/<loan_id>/1_2_4_llama_extractions/extraction_summary.json")
        sys.exit(1)
    ...
    # Run extraction
    summary = process_loan_text_no_tables(loan_id, stage1_output_dir, documents_dir)
