#!/usr/bin/env python3
"""
Batch Credit Score Verification
Runs credit score verification for all loans in the system.
"""

import sys
from db import execute_query
from summary_s4c_verify_credit_score import main as verify_credit_main


def batch_verify_credit():
    """Run credit verification for all loans."""
    print("=" * 80)
    print("🔍 BATCH CREDIT SCORE VERIFICATION")
    print("=" * 80)
    ...
    for idx, loan in enumerate(loans, 1):
        ...
        try:
            # Run verification
            success = verify_credit_main(loan_id)
            
            if success:
                results['success'].append(loan_id)
                print(f"✅ Loan {loan_id}: Credit verification completed successfully")
            else:
                results['skipped'].append(loan_id)
                print(f"⚠️  Loan {loan_id}: Skipped (no credit score or credit report)")
